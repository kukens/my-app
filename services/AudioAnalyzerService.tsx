...
export class AudioService {
    ...
    public async initMicrophone() {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        this.source = this.AudioContext.createMediaStreamSource(stream);
    }

    public startAnalysis(analysisResultCallback: (analysisResult: AnalysisResult) => void) {
        if (!this.source) throw new Error('Audio source not initialized');
        ...
        this.analyzer = Meyda.createMeydaAnalyzer({
            ...
            callback: (features: MeydaFeaturesObject) => {
                const spectrumInfo = this.anylyzeAudio(features.amplitudeSpectrum);
                analysisResultCallback(spectrumInfo);
            }
        });
